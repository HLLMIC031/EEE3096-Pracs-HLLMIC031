...
int main(void)
{
  ...
  // TODONE: Start timer TIM16
  HAL_TIM_Base_Start_IT(&htim16);


  // TODONE: Write all "patterns" to EEPROM using SPI
  ...
  while (1)
  {
	...
	// TODONE: Check button PA0; if pressed, change timer delay
	  if(PA0_Pressed){
		  if(boolState==0){
			  TIM16 -> ARR = 500-1;
			  boolState=1;
...
void TIM16_IRQHandler(void)
{
	...
	// TODONE: Change to next LED pattern; output 0x01 if the read SPI data is incorrect

	uint8_t outputPattern= read_from_address(patternIndex);

	if((outputPattern==patterns[patternIndex])){
		GPIOB -> ODR = outputPattern;
		if(patternIndex<5){
			patternIndex++;
		}else{
			patternIndex=0;
