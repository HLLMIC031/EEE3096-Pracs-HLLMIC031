...
void EXTI0_1_IRQHandler(void)
{
	// TODO NE: Add code to switch LED7 delay frequency
	if(boolean){
		delay_t=500;
		boolean=0;
	} else {
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	HAL_ADC_Start(&hadc);
	HAL_ADC_PollForConversion(&hadc, delay_t);
	HAL_ADC_Stop(&hadc);
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	uint32_t val=(int)floor(adc_val*(47999/4095));
	return val;
}
