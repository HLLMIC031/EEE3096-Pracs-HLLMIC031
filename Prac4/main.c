...
uint32_t TIM2_Ticks = (int)round(TIM2CLK/(NS*F_SIGNAL)); // How often to write new LUT value
uint32_t DestAddress = (uint32_t) &(TIM3->CCR3); // Write LUT TO TIM3->CCR3 to modify PWM duty cycle

uint32_t LUTstate=(uint32_t)&Sin_LUT;
...
int main(void)
{
  ...
  HAL_DMA_Start_IT(&hdma_tim2_ch1,LUTstate,DestAddress,NS);
...
void EXTI0_1_IRQHandler(void)
{
	...
	if((HAL_GetTick()-prevtick)>500){
		...
		case 0:
			trilean=1;
			LUTstate=(uint32_t)&saw_LUT;

			lcd_putstring("Saw");
		break;
		case 1:
			trilean=2;
			LUTstate=(uint32_t)&triangle_LUT;

			lcd_putstring("Triangle");
		break;
		case 2:
			trilean=0;
			LUTstate=(uint32_t)&Sin_LUT;
